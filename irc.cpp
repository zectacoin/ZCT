...
// Copyright (c) 2011-2012 Zecta Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #ZectaTEST0\r");
            Send(hSocket, "WHO #ZectaTEST0\r");
        } else {
            // randomly join #Zecta00-#Zecta99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #Zecta%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Zecta%02d\r", channel_number).c_str());
        }
