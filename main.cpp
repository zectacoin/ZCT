...
const string strMessageMagic = "Zecta Signed Message:\n";
...
{
    // Zecta: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // Zecta: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xda, 0xca, 0xaf, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZectaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZectaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZectaMiner(CWallet *pwallet)
{
    printf("ZectaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Zecta-miner");
    ...
    try { loop {
        ...
        printf("Running ZectaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZectaMiner, pwallet));
}
